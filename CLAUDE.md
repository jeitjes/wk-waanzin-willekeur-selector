...
- `wereldlied.html` (→ `/wereldlied`) — the Wereldlied game; starting/stopping a round is public, but admins (same key mechanism) get a 🗑 button per submitted line, backed by the admin-only `POST /api/wereldlied/verwijder {index}`. A deleted entry stays in `regels` as a placeholder (`overgeslagen: true, verwijderd: true`) so the turn↔refrein-line mapping stays intact and the original refrain line applies again — never splice `regels`.
